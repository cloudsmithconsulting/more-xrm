...
import { formatDynamicsResponse, AuthenticationType } from "./DynamicsRequest";
import * as httpntlm from "httpntlm";
import fetch from "node-fetch";
export function dynamicsBatch(connectionOptions, headers) {
    return new Batch(connectionOptions, headers);
}
export function dynamicsBatchRequest(connectionOptions, ...url) {
    const batch = new Batch(connectionOptions);
    batch.requestAllUrls(url);
    return batch.execute();
}
export function dynamicsBatchQuery(connectionOptions, ...query) {
    const batch = new Batch(connectionOptions);
        ...
        if (connectionOptions.authType === AuthenticationType.Windows) {
            return new Promise((resolve, reject) => {
                httpntlm.post({
                    ...
                    headers: Object.assign(Object.assign({ 'Content-Type': `multipart/mixed;boundary=batch_${batchId}` }, DynamicsHeaders), headers)
                }, function (err, res) {
                    if (err) {
                        console.error(err);
                        reject(err);
            ...
            return fetch(callUrl, {
                method: 'POST',
                headers: Object.assign(Object.assign({ 'Content-Type': `multipart/mixed;boundary=batch_${batchId}`, 'Authorization': connectionOptions.accessToken }, DynamicsHeaders), headers),
                body: Batch.formatBatchRequest(connectionOptions, batchId, requests)
            })
